...
*/
    ...
    override fun onListFragmentInteraction(item: SortObject) {
        ...
        if(item.child != null && item.child!![0].child != null && !TextUtils.equals(item.name, "Все")) {
            navigator?.navigateTo(FilterMainNavigator.FilterScreens.MAIN_FILTER, getItems(ArrayList(item.child?.toMutableList()), item.id))
        } else {
